			...
			<div class="recommend" :style='{"padding":"0","margin":"0px auto","flexWrap":"wrap","background":"none","display":"flex","width":"100%","position":"relative"}'>
				...
				<form class="layui-form filter" :style='{"padding":"30px 20px 30px","borderColor":"#bcdbdf","margin":"20px 7% 0","alignItems":"center","background":"url(http://codegen.caihongy.cn/20230206/544f501163914411bf89e5cb898e10ac.png) no-repeat center bottom,linear-gradient(30deg, rgba(255,255,255,1) 0%, rgba(207,193,176,1) 20%, rgba(246,236,223,1) 50%, rgba(207,193,176,1) 80%, rgba(255,255,255,1) 100%),#fff","borderWidth":"0px 0px 0px","display":"flex","width":"100%","borderStyle":"outset","justifyContent":"center","height":"auto"}'>
					...
					<div :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}' class="item-list">
						<div class="lable" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px"}'>教练姓名</div>
						<input type="text" :style='{"border":"1px solid #eee","padding":"0 10px","boxShadow":"0px 0px 0px #ccc","margin":"0","outline":"none","color":"#666","borderRadius":"30px","width":"140px","fontSize":"14px","lineHeight":"40px","height":"40px"}' name="jiaolianxingming" id="jiaolianxingming" placeholder="教练姓名" autocomplete="off" class="layui-input">
					</div>
					<div :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}' class="item-list">
						<div class="lable" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px"}'>教练性别</div>
						<select name="jiaolianxingbie" id="jiaolianxingbie" lay-filter="jiaolianxingbie">
							<option value="">请选择</option>
							<option v-for="(item,index) in jiaolianxingbieOptions" v-bind:key="index" :value="item">{{item}}</option>
						</select>
					</div>
					<div :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}' class="item-list">
						<div class="lable" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px"}'>可约时间</div>
						<select name="shijianduan" id="shijianduan" lay-filter="shijianduan">
							<option value="">请选择</option>
							<option v-for="(item,index) in shijianduanOptions" v-bind:key="index" :value="item">{{item}}</option>
						</select>
					</div>
		...
		<script>
			...
			layui.use(['form', 'layer', 'element', 'carousel', 'laypage', 'http', 'jquery','laydate', 'slider'], function() {
				...
				function pageList() {
			        ...
			        if (jquery('#jiaolianxingbie').val()) {
			          param['jiaolianxingbie'] = jquery('#jiaolianxingbie').val() ? jquery('#jiaolianxingbie').val() : '';
			        }
				...
				if (jquery('#shijianduan').val()) {
					...
					// 获取列表数据
					http.request('jiaolianxinxi/list', 'get', param, function(res) {
