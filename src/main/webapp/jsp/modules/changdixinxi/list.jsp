...
<body>
	<div id="main-container">
		...
		<div id="breadcrumb-container">
			<h3 class="breadcrumb-title">场地信息管理</h3>
			<ol class="breadcrumb-list">
				...
				<li class="breadcrumb-item-two"><span>场地信息列表</span></li>
			</ol>
		</div>
		<!-- /Breadcrumb -->
		...
		<div id="center-container">
			...
			<div class="btns">
				...
				<button onclick="chartDialog1()" type="button" class="btn btn-info btn-statis" v-if="crossBtnControl('场地状态统计')"><i class="fa fa-database"></i><span>场地状态统计</span></button>
			</div>
            ...
            <div class="openBox" v-if="showType1">
                ...
                <div class="openView">
                    <div id="zhuangtaiMain1" class="graph" style="width: 70vw;height:50vh;"></div>
                </div>
            </div>
		</div>
	...
	<script>
		...
		function setDataRow(item,number){
			...
			var changditupianCell  = document.createElement('td');

			            var changditupianImg = document.createElement('img');
            changditupianImg.setAttribute('src',item.changditupian?(baseUrl+item.changditupian.split(",")[0]):'');
			changditupianImg.setAttribute('height',100);
			changditupianImg.setAttribute('width',100);
			changditupianCell.appendChild(changditupianImg);


			row.appendChild(changditupianCell);
			...
			crossBtn0.setAttribute("onclick","crossTable(" + JSON.stringify(item) + ",'changdiyuyue','','zhuangtai','该场地不可预约','使用中,闲置'.split(',')[0])");
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("changdixinxi/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function discussList(id) {
            ...
            window.location.href = "../discusschangdixinxi/list.jsp"
        }
		//清除会重复渲染的节点
        function clear(){
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"changdixinxi");
                        ...
                        success: function(data){
                            if(data.code == 0){
                                if(data.data != null){
                                    ...
                                    var myChart = echarts.init(document.getElementById('zhuangtaiMain1'),'macarons');
                                    var option = {};
                                    option = {
                                            title: {
                                                text: '场地状态统计',
